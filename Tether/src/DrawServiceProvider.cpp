...
const float DrawServiceProvider::default_zoom = 1.0f;

DrawServiceProvider::DrawServiceProvider(sf::Window* window, EntityPlayer* observing_entity, IGameMode* simulation)
	...
	, dsLogic(&simulation->pmLogic, &graphics2d)
{
	...
	camera->zoom=DrawServiceProvider::default_zoom;
...
void DrawServiceProvider::enableAABBDrawing(bool b){ enable_aabbs = b; }
void DrawServiceProvider::enableDebugScreen(bool b){ enable_debug = b; }
void DrawServiceProvider::enableHUD(bool b){ enable_hud = b; }

void DrawServiceProvider::enableThirdPerson(bool on)
{
	if(on)
	{
...
void DrawServiceProvider::setThirdPersonDistance(float meters)
{
...
bool DrawServiceProvider::isThirdPerson() const
{
	return camera->dist >= third_person_distance_min;
}

void DrawServiceProvider::drawGameOver()
{
	transformViewToGUI();
	glColor3f(1, 0, 0);
...
unique_ptr<Frustum> DrawServiceProvider::newFrustumApplyPerspective(	SimClock::time_point t,
															bool fresh,
...
void DrawServiceProvider::drawAvatarPerspective()
{
...
void DrawServiceProvider::drawHUD()
{
	glEnable(GL_TEXTURE_2D);
	transformViewToGUI();
	glColor3f(0, 1, 0);
...
vec3 DrawServiceProvider::forwardVec() { return camera->getForwardVector(); }
vec3 DrawServiceProvider::upVec(){ return camera->getUpVector(); }
float DrawServiceProvider::getAspectRatio() { return camera->width/camera->height; }
vec3 DrawServiceProvider::getCamPos(){ return camera->eye->offsetFromEntity(); /* TODO accurate for 3rd person? */ }
...
void DrawServiceProvider::doTransparentCallbacks(const SimClock::time_point& t,Frustum * viewFrustum, IWorld& iw)
{
