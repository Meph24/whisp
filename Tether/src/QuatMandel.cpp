		...
		for(int y=0;y<resolution;y++)
		{
			for(int x=0;x<resolution;x++)
			{
				...
				*/
				...
				quat dyn(x*4.0f/resolution-2.0f,y*4.0f/resolution-2.0f,0,0);//z*4.0f/resolution-2.0f,0);
				quat start(0,z*4.0f/resolution-2.0f,0,0);
				quat startMandel(x*4.0f/resolution-2.0f,y*4.0f/resolution-2.0f,z*4.0f/resolution-2.0f,0);

				quat work=dyn;
				quat mandel=startMandel;
				int i;
				for(i=0;i<iter;i++)
				{
					work=work*work+start;
					mandel=mandel*mandel+startMandel;
					quat temp=work;
					work=work*mandel;
					mandel=mandel*temp;
					if(work.absSq()>4.0f) break;
				...
				start=start*quat({1,0,0},90*TAU/360);
				quat work=start;
				int i;
				for(i=0;i<iter;i++)
				{
